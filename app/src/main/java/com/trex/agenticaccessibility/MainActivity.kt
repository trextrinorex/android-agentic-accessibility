...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val endpoint=EditText(this).apply{hint="OpenAI-compatible endpoint";setText(store.get("endpoint")?:"https://api.openai.com/v1/chat/completions")}; val model=EditText(this).apply{hint="Model";setText(store.get("model")?:"gpt-4o-mini")}; val key=EditText(this).apply{hint="API key";inputType=0x81}
        val start=Button(this).apply{text="Start Agent"}; val stop=Button(this).apply{text="Stop"}; val access=Button(this).apply{text="Accessibility Settings"}; val clear=Button(this).apply{text="Clear task history"}; val mic=Button(this).apply{text="🎙 Push to talk"}; val log=TextView(this).apply{text="Activity log\n"}
        listOf(status,task,privacy,endpoint,model,key,start,stop,access,clear,mic,log).forEach{root.addView(it)};setContentView(root)
        ...
        stop.setOnClickListener{controller.stop();status.text="■ Stopped"}; clear.setOnClickListener{controller.clearHistory()}
